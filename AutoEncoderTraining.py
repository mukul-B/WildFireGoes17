...
from GlobalValues import training_dir, model_path, RES_ENCODER_PTH, RES_DECODER_PTH, RES_OPT_PTH, BATCH_SIZE, EPOCHS, \
    LEARNING_RATE
from HyperparameterConfigs import config1, sweep_configuration1, config_goodResult, sweep_configuration_beta, \
    config_IOU, sweep_configuration_IOU, config_IOU_hl, config_IOU_vhl
from LossFunctions import IMSE, IoULoss
...
def main(config=None):
    ...
    run = wandb.init(project="wildfire-project_IOU", name="run_" + current_time)
    if config:
        wandb.config = config
...
if __name__ == "__main__":
    ...
    # wandb.login()
    # sweep_configuration = sweep_configuration_IOU
    # sweep_id = wandb.sweep(sweep=sweep_configuration, project='my-first-sweep_Iou')
    # wandb.agent(sweep_id, function=main, count=15)

    config = config_IOU_vhl
    main(config)
