...
def createDataset(location, product_name=RAD):
    ...
    for i in range(time_dif.days):
        ...
        for ac_time in unique_time:
            ...
            # path = goes.download_goes(fire_date, str(ac_time), 'reference_data/' + location + '/GOES',
            ...
            if (path != -1):
                ...
                goes_tif_dir = goes_dir.replace('$LOC', location).replace('$PROD', product_name)
                goes.nc2tiff(fire_date, ac_time, path, site, v2r_viirs.image_size, goes_tif_dir)
